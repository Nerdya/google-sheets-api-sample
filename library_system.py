...
def create_main_window():
    ...
    def create_book_widgets():
        ...
        menu_left.columnconfigure(1, weight=2)
        # Separator
        menu_left.rowconfigure(10, weight=1)

        label_arr = []
        entry_arr = []
        for i in range(len(book_label)):
            label_arr.append(ttk.Label(menu_left, text=book_label[i]))
            label_arr[i].grid(column=0, row=i, sticky=tk.W, padx=(10, 0), pady=10)
            entry_arr.append(ttk.Entry(menu_left, textvariable=book_var[i]))
        ...
        button = ttk.Button(menu_left, text='Tìm', compound=tk.LEFT, command=button_clicked)
        button.grid(column=0, row=11, sticky=tk.N, pady=10)
    ...
    # Create frames, configure the grid
